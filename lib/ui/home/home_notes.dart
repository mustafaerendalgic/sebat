...
import 'package:sebat/data/cubits/note_cubit.dart';
    ...
    return Scaffold(
      body: BlocBuilder<NoteCubit, NotesState>(
        ...
        builder: (context, state) {
          final Widget _body;
          _body = switch(state){
            NotesInitial() || NotesLoading() => Center(child: CircularProgressIndicator()),
            NotesError() => 
              Column(
                  spacing: 16,
                  children: [
                    LottieBuilder.asset("assets/ask_ai.json"),
                    Text("Somethings wrong: " + state.error)
                  ],
                ),
            NotesLoaded() =>
              CustomScrollView(
                ...
                slivers: [
                  ...
                  SliverList(
                    delegate: SliverChildBuilderDelegate(
                      ...
                      (context, index) {
                        final Note item = state.notes[index];
                        if (index != 0 &&
                            state.notes[index - 1].date.split(" ")[1] !=
                                state.notes[index].date.split(" ")[1]) {
                          return Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            spacing: 24,
                            children: [
